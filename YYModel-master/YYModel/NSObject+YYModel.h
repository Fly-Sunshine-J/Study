...
/**
 ...
 模型转json数据，如果是数组，字典，集合只会转换内部的数据
 */
...
/**
 ...
 If the reciver is `NSArray`, `NSDictionary` or `NSSet`, it will also convert the 
 inner object to json string.
 模型转json字符串， 如果是数组，字典，集合只会转换内部的数据
 */
...
/**
 ...
 这个方法会在+modelWithJSON:`, `+modelWithDictionary:`, `-modelSetWithJSON:` and `-modelSetWithDictionary:`.之前调用，如果返回nil，这个模型将会被忽略
 */
...
/**
 ...
 如果生成的模型不是你期望的，你可以实现这个方法验证模型的属性，当返回NO的时候，生成的这个模型是不可用的
 */
...
/**
 ...
 @return Returns YES if the model is valid, or NO to ignore this model.
 如果生成的json数据不是你想要的，你可以实现这个方法，验证这个json字典，如果返回NO会忽略这个模型
 */
