...
//    计算头部视图的大小，如果不是浮动模式，值为CGSizeZero
    self.headerReferenceSize = ({
        CGSize headerSize = CGSizeZero;
        if (self.calendar.floatingMode) {   //如果是浮动模式，计算section的头部高度
...
//    计算estimatedItemSize
    self.estimatedItemSize = ({
        ...
        CGFloat height = ({
            ...
            if (!self.calendar.floatingMode) {  //如果不是浮动模式
                switch (self.calendar.transitionCoordinator.representingScope) {
                    case FSCalendarScopeMonth: {    //如果视野范围是月份 预估高度
                        height = (self.collectionView.fs_height-self.sectionInsets.top-self.sectionInsets.bottom)/6.0;
                        break;
                    }
                    case FSCalendarScopeWeek: {     //如果视野范围是周 预估高度
    ...
    free(self.widths);
    self.widths = ({
        NSInteger columnCount = 7;  //7列
        size_t columnSize = sizeof(CGFloat)*columnCount; //使用指针
