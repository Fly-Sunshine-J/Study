...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        _coverView = [[UIImageView alloc] initWithFrame:CGRectMake(160, 2, 60, 40)];
...
//        [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:[NSURL URLWithString:@"http://gank.io/api/data/%E7%A6%8F%E5%88%A9/100/1"] options:0 progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished) {
//            NSDictionary *json = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:nil];
//            NSArray *results = json[@"results"];
//            for (NSDictionary *dict in results) {
//                [self.objects addObject:dict[@"url"]];
//            }
...
//                                     options:indexPath.row == 0 ? SDWebImageRefreshCached : 0];
    [cell.coverView sd_setImageWithURL:[NSURL URLWithString:self.objects[indexPath.row]]];
    return cell;
}
